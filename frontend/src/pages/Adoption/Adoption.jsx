...
const DogProfile = ({ dog, onClose, onSuccess }) => {
  const [showTerms, setShowTerms] = useState(false); // Add this state declaration
 ...
 const handleAdoptSubmit = async (data) => {
   try {
     ...
     const errorMessage =
       ...
       error.response?.data?.adoption_reason?.[0] ||
     ...
     console.error("Adoption Error:", error.response?.data || error.message);
   }
 };
  return (
